# Modified to use -LINE control statements to perform source error tracking.
# We no longer need the epilogue file.
#
	DATA("b(op,l,r)")
	DATA("u(op,r)")
	...
	clause = FENCE && ARBNO(label) && optblank &&
		("if" . cl_type && optblank && "(" && *expr && optblank && ")" |
		"while" . cl_type && optblank && "(" && *expr && optblank && ")" |
		...
		"for" . cl_type && optblank &&
			"(" && *expr && optblank && "," && *expr && optblank &&
			"," && *expr && optblank && ")" |
		RPOS(0) . cl_type && *invoke(.emiteos) |
		*expr && optblank && clausend . *invoke(.expcl))
...
#	if this is an MS-DOS or OS/2 system, need to use ".exe" extension.
#	also record the command (rm or del) to be used to delete a file.
        if (HOST() ? FENCE && BREAKX(":") && (":MS-DOS" | ":OS/2" |
          ":WinNT" | ":Win95")) {
		gl_extension = ".exe"
		gl_delete = "del "
		}
...
#	establish the "object" file
	...
	if (~OUTPUT(.outf,-1,outfile && extension)) {
		OUTPUT = "cannot write " && outfile && extension
		go to END
	}
	&ERRLIMIT = &ERRLIMIT - 1
	SETEXIT (savexit)
...
#
#	Permanent prologue.  Watch for SNOBOL4+ option "-+"
	if (gl_options ? "-+") {
		emitlab("-PLUSOPS 1")
		emiteos()
		}
...
EXIT:
	...
	# put out code to create the "real" load module if not SNOBOL4+
	emiteos()
	if (~(gl_options ? "-+")) {
...
#       Compile output file.  (removed)
#	If not SNOBOL4+
#        if (~(gl_options ? "-+")) {
#		Intel-extended SPITBOL cannot launch another copy of itself.
#                if (~(HOST() ? FENCE && "80386:MS-DOS(Intel Extended)")) {
#                        HOST(1, "spitbol " && gl_options && " -b " && outfile && extension)
#			HOST(1, gl_delete && outfile && extension)
#                }
#        }
...
#	If a command-line option beginning with "-" is encountered,
#	append it to gl_options.
procedure gl_nextfile() {
	gl_arg = gl_arg + 1
	while (IDENT (SUBSTR (HOST (2, gl_arg), 1, 1), "-")) {
...
procedure gl_open (file) t {
	...
	if (INPUT (.gl_in, gl_index, file) ||
	 INPUT (.gl_in, gl_index, (gl_file = file && ".sc"))) {
		SETEXIT(t)
		&ERRLIMIT = &ERRLIMIT - 1
		file ? IDENT(outfile) && (BREAK(".") | REM).outfile
		return
	}
	SETEXIT(t)
	gl_close()
...
procedure buildstab (stmt, file, line) desc, pad {
	if (DIFFER (file, bst_file))
		outf = "-LINE " && line && ' "' && file && '"'
	else if (DIFFER (line, bst_line))
		outf = "-LINE " && line
#
